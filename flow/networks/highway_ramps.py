...
class HighwayRampsNetwork(Network):
    ...
    * **next_off_ramp_proba** : probability for a vehicle to exit the highway
    ...
    def __init__(
        ...
        self.nodes_pos = sorted(list(set([0] + self.ramps_pos + [self.highway_length])))
        ...
        # is at position x
        self.ramp_pos = {
            x: "on_ramp_{}".format(i) for i, x in enumerate(self.on_ramps_pos)
        }
        self.ramp_pos.update(
            {x: "off_ramp_{}".format(i) for i, x in enumerate(self.off_ramps_pos)}
        )

        # make sure network is constructable
        if len(self.ramps_pos) > 0 and (
            min(self.ramps_pos) <= 0 or max(self.ramps_pos) >= self.highway_length
        ):
            raise ValueError(
                "All ramps positions should be positive and less "
    ...
    def specify_nodes(self, net_params):
        """See parent class."""
        nodes_highway = [
            {"id": "highway_{}".format(i), "x": self.nodes_pos[i], "y": 0, "radius": 10}
    ...
    def specify_routes(self, net_params):
        ...
        def get_routes(start_node_pos):
            ...
            if start_node_pos not in self.nodes_pos:
                raise ValueError("{} is not a node position.".format(start_node_pos))
