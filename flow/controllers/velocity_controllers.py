...
class FollowerStopper(BaseController):
    ...
    def __init__(self, veh_id, car_following_params, v_des=15, danger_edges=None):
        """Instantiate FollowerStopper."""
        BaseController.__init__(
            self, veh_id, car_following_params, delay=1.0, fail_safe="safe_velocity"
        )
    ...
    def get_accel(self, env):
        ...
        if (
            self.find_intersection_dist(env) <= 10
            and env.k.vehicle.get_edge(self.veh_id) in self.danger_edges
            or env.k.vehicle.get_edge(self.veh_id)[0] == ":"
        ):
            return None
        else:
...
class PISaturation(BaseController):
    ...
    def get_accel(self, env):
        ...
        v_target = v_des + self.v_catch * min(
            max((dx - self.g_l) / (self.g_u - self.g_l), 0), 1
        )
        ...
        self.v_cmd = (
            beta * (alpha * v_target + (1 - alpha) * lead_vel) + (1 - beta) * self.v_cmd
        )
