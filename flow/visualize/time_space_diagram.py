...
def import_data_from_trajectory(fp, params=dict()):
    ...
    df[["next_pos", "next_time"]] = df.groupby("id")[["distance", "time_step"]].shift(
        -1
    )
...
def get_time_space_data(data, params):
    ...
    assert (
        params["network"] in ACCEPTABLE_NETWORKS
    ), "Network must be one of: " + ", ".join(
        [network.__name__ for network in ACCEPTABLE_NETWORKS]
    )
...
def _merge(data):
    ...
    segs = data[["time_step", "distance", "next_time", "next_pos"]].values.reshape(
        (len(data), 2, 2)
    )
...
def _i210_subnetwork(data):
    ...
    for lane, df in data.groupby("lane_id"):
        segs[lane] = df[
            ["time_step", "distance", "next_time", "next_pos"]
        ].values.reshape((len(df), 2, 2))
...
def _get_abs_pos(df, params):
    ...
    if params["network"] == MergeNetwork:
        ...
        ring_edgelen = ring_radius * np.pi / 2.0
    ...
    if params["network"] == FigureEightNetwork:
        ...
        figure_eight_len = (
            6 * ring_edgelen + 2 * intersection + 2 * junction + 10 * inner
        )
        intersection_loc = [
            edgestarts[":center_1"] + intersection / 2,
            edgestarts[":center_0"] + intersection / 2,
        ]
        ret.loc[ret < intersection_loc[0]] += figure_eight_len
        ret.loc[
            (ret > intersection_loc[0]) & (ret < intersection_loc[1])
        ] += -intersection_loc[1]
...
def plot_tsd(ax, df, segs, args, lane=None, ghost_edges=None, ghost_bounds=None):
    ...
    if ghost_edges:
        ...
        rects.append(
            Rectangle(
                (xmin, y_domain_min), args.start - xmin, y_domain_max - y_domain_min
            )
        )
        ...
        rects.append(
            Rectangle((xmin, ghost_bounds[1]), xmax - xmin, ymax - ghost_bounds[1])
        )
    else:
...
if __name__ == "__main__":
    ...
    # required arguments
    parser.add_argument(
        "trajectory_path", type=str, help="path to the Flow trajectory csv file."
    )
    parser.add_argument(
        "flow_params", type=str, help="path to the flow_params json file."
    )

    # optional arguments
    parser.add_argument(
        "--steps", type=int, default=1, help="rate at which steps are plotted."
    )
    parser.add_argument(
        "--title",
        type=str,
        default="Time Space Diagram",
        help="rate at which steps are plotted.",
    )
    parser.add_argument(
        "--max_speed", type=int, default=8, help="The maximum speed in the color range."
    )
    parser.add_argument(
        "--min_speed", type=int, default=0, help="The minimum speed in the color range."
    )
    parser.add_argument(
        "--start", type=float, default=0, help="initial time (in sec) in the plot."
    )
    ...
    if flow_params["network"] == "MergeNetwork":  #
        plt.plot(
            [df["time_step"].min(), df["time_step"].max()],
            [0, 0],
            linewidth=3,
            color="white",
        )  #
