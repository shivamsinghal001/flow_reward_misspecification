...
class TraCIKernelNetwork(BaseKernelNetwork):
    ...
    def generate_network(self, network):
        ...
        assert (
            self.network.net_params.template is None
        ...
        self.__length = sum(self._edges[edge_id]["length"] for edge_id in self._edges)
        ...
        cfg_name = self.generate_cfg(
            self.network.net_params, self.network.traffic_lights, self.network.routes
        )
    ...
    def generate_net(
        ...
        * connections (optional): describes how incoming and outgoing edge/lane
        ...
        for node in nodes:
            if node["id"] not in tl_ids and node.get("type", None) == "traffic_light":
        ...
        for edge in edges:
            ...
            if "shape" in edge:
                if not isinstance(edge["shape"], str):
                    edge["shape"] = " ".join(
                        "%.2f,%.2f" % (x, y) for x, y in edge["shape"]
                    )
            if "numLanes" in edge:
        ...
        x = makexml(
            "configuration", "http://sumo.dlr.de/xsd/netconvertConfiguration.xsd"
        )
        t = E("input")
    ...
    def generate_net_from_osm(self, net_params):
        ...
        net_cmd = "netconvert --osm-files {0} --output-file {1}".format(
            osm_path, self.cfg_path + netfn
        )
    ...
    def generate_cfg(self, net_params, traffic_lights, routes):
        ...
        for params in self.network.vehicles.types:
            type_params_str = {
                key: str(params["type_params"][key]) for key in params["type_params"]
            }
        ...
        if num_traffic_lights > 0:
            if traffic_lights.baseline:
                ...
                for node in nodes:
                    tll.append(
                        {"id": node["id"], "type": tl_type, "programID": program_id}
                    )
        ...
        routes_data = makexml("routes", "http://sumo.dlr.de/xsd/routes_file.xsd")
        ...
        for route_id in routes.keys():
            ...
            for i in range(len(routes[route_id])):
                ...
                routes_data.append(
                    E("route", id="route{}_{}".format(route_id, i), edges=" ".join(r))
                )
        ...
        cfg.append(
            _inputs(net=self.netfn, add=self.addfn, rou=self.roufn, gui=self.guifn)
        )
        t = E("time")
    ...
    def _import_edges_from_net(self, net_params):
        ...
        net_path = (
            os.path.join(self.cfg_path, self.netfn)
            if net_params.template is None
            else self.netfn
        )
            ...
            if "type" in edge.attrib and edge.attrib["type"] in types_data:
                if net_data[edge_id]["speed"] is None:
                    net_data[edge_id]["speed"] = float(
                        types_data[edge.attrib["type"]]["speed"]
                    )
