...
class PygletRenderer(object):
    ...
    def __init__(
        ...
        for lane_poly in self.lane_polys:
            lane_poly[::2] = [
                (x - self.x_shift) * self.x_scale * self.pxpm for x in lane_poly[::2]
            ]
            lane_poly[1::2] = [
                (y - self.y_shift) * self.y_scale * self.pxpm for y in lane_poly[1::2]
            ]
            color = [
                c
                for _ in range(int(len(lane_poly) / 2))
        ...
        try:
            ...
            print("Rendering with frame {} x {}...".format(self.width, self.height))
        except ImportError:
            self.window = None
            self.frame = None
    ...
    def render(
        ...
        self._add_vehicle_polys(human_orientations, human_conditions, 0)
        if self.show_radius:
            self._add_vehicle_polys(
                machine_orientations, machine_conditions, self.sight_radius
            )
        else:
            self._add_vehicle_polys(machine_orientations, machine_conditions, 0)
        ...
        if self.save_render:
            cv2.imwrite("%s/frame_%06d.png" % (self.path, self.time), self.frame)
            self.data.append(
                [
                    _human_orientations,
    ...
    def get_sight(self, orientation, veh_id):
        ...
        cv2.circle(
            mask,
            (int(sight_radius), int(sight_radius)),
            int(sight_radius),
            (255, 255, 255),
            thickness=-1,
        )
        ...
        if self.save_render:
            cv2.imwrite(
                "%s/sight_%s_%06d.png" % (self.path, veh_id, self.time), rotated_sight
            )
        if "gray" in self.mode:
    ...
    def _add_triangle(self, center, angle, size, color):
        ...
        pt1 = [cx, cy]
        pt1_ = [
            cx - s * self.x_scale * np.sin(ang),
            cy - s * self.y_scale * np.cos(ang),
        ]
        pt2 = [
            pt1_[0] + 0.25 * s * self.x_scale * np.sin(np.pi / 2 - ang),
