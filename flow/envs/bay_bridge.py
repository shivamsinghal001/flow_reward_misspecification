...
class BayBridgeEnv(Env):
    ...
    def __init__(self, env_params, sim_params, network, simulator="traci", path=None):
        ...
        self.toll_wait_time = np.abs(
            np.random.normal(
                MEAN_SECONDS_WAIT_AT_TOLL / self.sim_step,
                4 / self.sim_step,
                NUM_TOLL_LANES,
            )
        )
    ...
    def apply_toll_bridge_control(self):
            ...
            for car in cars_in_lane:
                ...
                if pos > TOLL_BOOTH_AREA:
                    if veh_id not in self.cars_waiting_for_toll:
                        if self.simulator == "traci":
                            # Disable lane changes inside Toll Area
                            lc_mode = self.k.kernel_api.vehicle.getLaneChangeMode(
                                veh_id
                            )
                            self.k.kernel_api.vehicle.setLaneChangeMode(veh_id, 512)
                        else:
        ...
        if new_tls_state != self.tl_state:
            ...
            self.k.traffic_light.set_state(node_id=TB_TL_ID, state=new_tls_state)
