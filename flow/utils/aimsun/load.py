...
def get_dict_from_objects(sections, nodes, turnings, cen_connections):
    ...
    centroid_config = model.find_by_name(
        model.centroid_configurations, centroid_config_name
    )
    if not centroid_config:
        print(
            "[load.py] ERROR: Centroid configuration "
    ...
    # load nodes
    for n in nodes:
        scenario_data["nodes"][n.id] = {"name": n.name, "nb_turnings": len(n.turnings)}
...
file_path = os.path.join(
    config.PROJECT_PATH, "flow/utils/aimsun/aimsun_template_path_%s" % port_string
)
...
with open(scenario_data_path, "w") as f:
    ...
    print("[load.py] Template's scenario data written into " + scenario_data_path)
